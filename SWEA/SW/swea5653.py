...
for tc in range(1, T+1):
    ...
    w_c = defaultdict(dict)  # key: 활성 시간 | value: 딕셔너리 => key: 크기 | value: 좌표(리스트)
    a_c = defaultdict(dict)  # key: 죽는 시간 | value: 딕셔너리 => key: 크기 | value: 좌표(리스트)
    for i in range(N*M):  # 초기 상태의 대기 세포 갱신
        r, c = divmod(i, M)
        if A[r][c]:
            d_c.add((r, c))  # 중복 방지를 위해 미리 추가
            if A[r][c] in w_c[A[r][c]]:
                w_c[A[r][c]][A[r][c]].append((r, c))
            else:
                w_c[A[r][c]][A[r][c]] = [(r, c)]
    ...
    for i in range(1, K+1):
        go = dict()
        for j in new:  # 활성화된 세포 중 번식 가능한 좌표 확인
            for vk, vv in a_c[j].items():
                for x, y in vv:
        ...
        for k, v in go.items():  # 세포 번식
            ...
            if v in w_c[i+v]: w_c[i+v][v].append(k)
            else: w_c[i+v][v] = [k]
